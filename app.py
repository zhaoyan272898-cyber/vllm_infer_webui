...
# from pdf_export import export_to_pdf, export_pdf   # 保留旧导出以备不时之需
...
#
#     if not files:
#         yield gr.update(value=[]), "未选择图片", "0s", "0s", None, {"image_paths": [], "result_texts": [], "question": ""}
#         return
#
#     file_paths = [f.name for f in files]
#     image_paths = get_image_files(file_paths)
#     if not image_paths:
#         yield gr.update(value=[]), "未找到图片文件", "0s", "0s", None, {"image_paths": [], "result_texts": [], "question": ""}
#         return
#
#     model = load_model(MODEL_PATH)
#     total = len(image_paths)
#     results = []          # DataFrame 用
#     result_texts = []     # 纯结果文本列表
#     total_time = 0.0
#
...
#
#         # 保存到列表
#         results.append([os.path.basename(img_path), result_text, elapsed])
#         result_texts.append(result_text)
#         progress((idx + 1) / total, desc=f"{LANG[current_lang]['progress_desc']} {idx+1}/{total}")
#
#         df = pd.DataFrame(results, columns=LANG[current_lang]["output_headers"])
#         gpu_info_str = f"显存: {model.gpu_info().get('Allocated(GB)', 0)} GB / {model.gpu_info().get('Total(GB)', 0)} GB"
#
#         # 构建 state_dict：包含所有已处理的图片路径、结果和问题
#         state_dict = {
#             "image_paths": image_paths[:idx+1],   # 已处理的图片路径（也可全部路径，但未处理的无结果，建议只传已处理的）
#             "result_texts": result_texts,
#             "question": prompt,
#         }
#
#         yield df, f"已处理 {len(results)}/{total}", f"{total_time:.3f}s", gpu_info_str, state_dict
#
#     # 完成
#     final_status = LANG[current_lang]["complete"] if not stop_flag else "已停止"
#     # 最终状态也更新一次
#     final_state_dict = {
#         "image_paths": image_paths[:len(results)],
#         "result_texts": result_texts,
#         "question": prompt,
#     }
#     yield df, final_status, f"{total_time:.3f}s", gpu_info_str, final_state_dict
#
# # ---------- 导出函数（带图片） ----------
# def export_pdf_with_images(state_dict):
#     """从 state_dict 中取数据，生成带图片的 PDF"""
#     if not state_dict or not state_dict.get("image_paths"):
#         return None
#     image_paths = state_dict["image_paths"]
#     result_texts = state_dict["result_texts"]
#     question = state_dict.get("question", "")
#     # 注意：export_to_pdf 返回文件路径
#     pdf_path = export_to_pdf(image_paths, result_texts, question)
#     return pdf_path
# def export_excel_from_state(state_dict):
#     if not state_dict or not state_dict.get("image_paths"):
#         return None
...
#         return tmp.name
#
# # 其他导出（Excel, Markdown）不变
# def export_excel_fn(results_df: pd.DataFrame):
#     if results_df is None or results_df.empty:
#         return None
#     with tempfile.NamedTemporaryFile(suffix=".xlsx", delete=False) as tmp:
...
#         return tmp.name
#
# def export_md_fn(results_df: pd.DataFrame):
#     if results_df is None or results_df.empty:
#         return None
#     md = "# Qwen3.5-VL 推理结果\n\n"
...
#
#         with gr.Column(scale=1):
#             gpu_status_label = gr.Label(value="GPU 状态")
#             gpu_status = gr.Label(value="等待推理...")
#             model_status_label = gr.Label(value="模型状态")
#             model_status = gr.Label(value="未加载")
#             time_info_label = gr.Label(value="推理耗时")
#             time_info = gr.Label(value="0s")
...
#         export_md_btn = gr.DownloadButton("📝 导出 Markdown")
#
#     # 状态：存储当前推理数据（图片路径、结果、问题）
#     state_dict = gr.State(value={"image_paths": [], "result_texts": [], "question": ""})
#     state_df = gr.State(value=None)   # 不再使用，保留以防
#
#     # ---------- 事件绑定 ----------
#     upload.change(update_gallery, inputs=[upload], outputs=[gallery, state_dict])
#
...
#         inference_process,
#         inputs=[upload, prompt, temperature, top_p, max_tokens, batch_size],
#         outputs=[output_df, model_status, time_info, gpu_status, state_dict],
#         queue=True,
#     )
#
#     stop_btn.click(stop_inference, outputs=[model_status])
...
#         export_excel_fn,
#         inputs=[state_df],   # 这里我们需要修改为 output_df 的数据？实际上 state_df 未更新，最好使用 state_dict 构建 DataFrame
#         outputs=[export_excel_btn],
#     )
#     # 更稳健的做法：导出 Excel 时直接从 state_dict 构建 DataFrame
#     # 但由于我们保留了 state_df 无用，可以改为使用 state_dict
#     # 这里简单处理：将 export_excel_fn 修改为接收 state_dict
#     # 我将在下面重新定义 export_excel_from_state
#     export_excel_btn.click(export_excel_from_state, inputs=[state_dict], outputs=[export_excel_btn])
#     export_md_btn.click(export_md_from_state, inputs=[state_dict], outputs=[export_md_btn])
#     # 导出 Markdown 同理
#     # 为了简洁，这里不细改，您可自行调整。
